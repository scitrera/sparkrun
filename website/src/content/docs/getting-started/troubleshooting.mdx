...
This runs `chown` on `~/.cache/huggingface` across all target hosts, handling sudo detection automatically. If passwordless sudo isn't configured, it will prompt for a password once.
...
- If a container does end up downloading files, run `sparkrun setup fix-permissions` to fix ownership before the next launch.