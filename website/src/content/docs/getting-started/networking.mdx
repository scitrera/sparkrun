...
Plug in your QSFP56 cable and check which interfaces come up:

```bash
ibdev2netdev
```

The "Up" interfaces correspond to your connected port. Note the twin pair names — you will need both.

### 2. Create a netplan configuration

Create `/etc/netplan/40-cx7.yaml` on **each node** with the appropriate IPs. Use a consistent addressing scheme across all nodes.
...
network:
  ...
  ethernets:
    enp1s0f1np1:
      ...
      link-local: [ ipv4 ]
      mtu: 9000
      addresses: [192.168.11.14/24]
    enP2p1s0f1np1:
      dhcp4: no
      dhcp6: no
      link-local: [ ipv4 ]
      mtu: 9000
      addresses: [192.168.12.14/24]
```
...
Ping across both subnets from each node:
...
Verify MTU is effective end-to-end:

```bash
ping -M do -s 8972 -c 3 192.168.11.14
```

A successful large-packet ping confirms jumbo frames are working. If it fails, check MTU on both endpoints and any switch in between.
