...
class VllmTuner:
    ...
    def run_tuning(
        ...
        logger.info("=" * 60)
        logger.info("sparkrun vLLM Kernel Tuner")
        logger.info("=" * 60)
    ...
    def _launch_container(self) -> int:
        ...
        volumes[self.output_dir] = VLLM_TUNING_CONTAINER_OUTPUT_PATH
    ...
    def _clone_benchmarks(self) -> int:
        """Step 2: Clone vLLM benchmark scripts inside the container."""
        import time
        from sparkrun.orchestration.primitives import run_script_on_host
        ...
        t0 = time.monotonic()
        logger.info("Step 2/5: Cloning vLLM benchmark scripts...")

        clone_script = read_script("vllm_clone_benchmarks.sh")
        exec_cmd = docker_exec_cmd(TUNE_VLLM_CONTAINER_NAME, clone_script)
    ...
    def _run_tune_for_tp(self, tp_size: int, triton_version: str) -> int:
        ...
        exec_cmd = docker_exec_cmd(TUNE_VLLM_CONTAINER_NAME, tune_cmd)
    ...
    def _cleanup_container(self) -> None:
        ...
        cmd = docker_stop_cmd(TUNE_VLLM_CONTAINER_NAME)
        run_command_on_host(
            self.host, cmd,
            ssh_kwargs=self.ssh_kwargs, timeout=30, dry_run=self.dry_run,
        )
